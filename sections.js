...
const getallcoches = async (page)=>{
    ...
    const call = await fetch(`https://back-taller-mecanico.vercel.app/coches?page=${page}&limit=5`, {
        method: 'GET', // or 'PUT'
...
const getallservicios = async (page)=>{
    ...
    container$$.innerHTML=``;
    let token = localStorage.getItem ("token");
    const call = await fetch(`https://back-taller-mecanico.vercel.app/servicios?page=${page}&limit=5`, {
        method: 'GET', // or 'PUT'
...
const printresults = async (allJson,tipo)=>{
    //console.log(tipo)
    if (tipo === "coches"){
        const call = await fetch("https://back-taller-mecanico.vercel.app/clientes/all");
...
const sendNewcoche = async (event)=>{
    ...
    let token = localStorage.getItem ("token");
    const call = await fetch("https://back-taller-mecanico.vercel.app/coches", {
        method: 'POST',
...
const deletecoche = async ()=>{
    const call= await fetch("https://back-taller-mecanico.vercel.app/coches/all")
    const allcochesJson = await call.json();
...
const cocheaborrar = async ()=>{
    ...
    const call = await fetch(`https://back-taller-mecanico.vercel.app/coches/id/${event.target.value}`)
...
const borrarCoche = async (id)=>{
    maincontainer$$.innerHTML=``;
    const call=await fetch(`https://back-taller-mecanico.vercel.app/coches/${id}`,{
        method: 'DELETE'
    })
